...
if os.path.exists(default_openms_data_path):
    if not env_openms_data_path:
        ...
        logger.warning(
            "OPENMS_DATA_PATH environment variable already exists. "
    ...
    if not env_openms_data_path:
         logger.warning(
            "OPENMS_DATA_PATH environment variable not found and no share directory was installed. "
...
======================================================================
""".format(pyopenms_so_path=os.path.join(here, "pyopenms*.so")))
    ...
    try:
        ...
        logger.error("""PyQt5 was found to be installed. When building pyopenms qmake said:

{info}

PYQT has version {pyqt_version}

This might cause a conflict if both are loaded.  You can test this by importing pyopenms
first and then import PyQt5.QtCore.

Note: when using the Spyder IDE, the usage of PyQt might be circumvented
by not using the 'Automatic' backend. Please change this in

Tools -> Preferences -> IPython -> Graphics to 'Inline'

In general, try to install everything with conda in the same environment to make sure Qt is used in the same version.

======================================================================
""".format(info=info, pyqt_version=PyQt5.QtCore.PYQT_VERSION_STR))
